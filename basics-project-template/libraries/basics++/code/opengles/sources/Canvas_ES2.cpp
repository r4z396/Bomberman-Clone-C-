...
namespace basics { namespace opengles
{
    ...
    void Canvas_ES2::fill_rectangle (const Point2f & where, const Size2f & size, const basics::Texture_2D * texture, int anchor)
    {
        ...
        void Canvas_ES2::fill_rectangle(const Point2f &where, const Size2f &size, const basics::Texture_2D *texture, const Point2f &positionUse, const Size2f &sizeUse, int anchor)
        {
            const opengles::Texture_2D * opengl_es_texture = dynamic_cast< const opengles::Texture_2D * >(texture);
            ...
            if (opengl_es_texture)
            {
                Point2f bottom_left
                        {
                            positionUse.coordinates.x(),
                            positionUse.coordinates.y()
                ...
                {
                    case LEFT:   bottom_left[0] = where[0];                  break;
                    case CENTER: bottom_left[0] = where[0] - sizeUse[0] * 0.5f; break;
                    case RIGHT:  bottom_left[0] = where[0] - size[0];        break;
                }
                ...
                {
                    case TOP:    bottom_left[1] = where[1] - size[1];        break;
                    case CENTER: bottom_left[1] = where[1] - sizeUse[1] * 0.5f; break;
                    case BOTTOM: bottom_left[1] = where[1];                  break;
                }
                ...
                Point2f top_right
                        {
                                bottom_left.coordinates.x () + sizeUse.width,
                                bottom_left.coordinates.y () + sizeUse.height
                        };
